...
class ICUReader:
	def __init__(self, path, framerate=30, timeOffset=0, clockTime=0, cameraNumber=0, viz=1, vizSkel=0):
		self.path = path
		...
		if currentDirTime > 0:
			...
			if len(files) > 0:
				...
				if len(depthFilename) > 0:
					...
					self.depthData = np.array(depthRaw, dtype=int).reshape([480,640])
					self.depthData = self.depthData[:,-1:0:-1]
